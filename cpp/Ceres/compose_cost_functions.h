   ...
   public:
    ...
    int AddInternalParameterBlock(double* p, int block_size) {
      ...
      if (i != parameter_blocks_.end()) {
        j = (int)std::distance(parameter_blocks_.begin(), i);
      } else {
        parameter_blocks_.push_back(p);
        parameter_block_sizes_.push_back(block_size);
        j = (int)parameter_blocks_.size() - 1;
      }
      return j;
    }
    ...
    virtual bool Evaluate(const double * const* x, double* r, double** J) const {
      ...
      const int num_g_parameter_blocks = (int)g_parameter_block_sizes.size();
      ...
      for (auto& f : f_)
        num_jacobian_blocks += (int)std::get<3>(f).size();
      ...
      for (size_t i = 0; i < f_.size(); ++i) {
        ...
        J_i_cursor += (int)std::get<3>(f_[i]).size();
      }
